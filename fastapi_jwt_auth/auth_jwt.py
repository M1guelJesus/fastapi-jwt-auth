...
class AuthJWT(AuthConfig):
    ...
    def _verify_jwt_in_request(
        ...
        if token_from not in ["headers", "cookies", "websocket", "body"]:
            raise ValueError(
                "token_from must be between 'headers', 'cookies', 'websocket', 'body'"
            )
        ...
        if not token:
            ...
            if token_from == "body":
                raise MissingTokenError(
                    status_code=401,
                    message="Missing {} token from Body".format(type_token),
                )
...
class AuthJWTRefresh(AuthJWT):
    def __init__(
        self, req: Request = None, res: Response = None, refresh_token: BodyToken = None
    ):
